...
def main(both=False, user=True):
    ...
    if (both):
        ...
        if (user):
            user_tracks = begin_build(
                sp, oauth, user_flag=True, with_audio=True)
            return user_tracks
        else:
            seed_tracks = begin_build(
                sp, oauth, user_flag=False, with_audio=True)
            return seed_tracks


def begin_build(client: spotipy.Spotify, oauth: SpotifyOAuth, 
                user_flag: bool, with_audio=True):
    # do first page of playlists
    playlists_idx = []
    playlists = client.current_user_playlists()
    process_page(playlists=playlists, user_flag=user_flag,
                 client=client, playlist_idx=playlists_idx)
    # do more if needed
    if (playlists['next']):
        ...
        while playlists['next']:
            playlists = requests.get(playlists['next'],
    ...
    df_with_audio = sp_utility.get_tracks(
        client, oauth, ("p", playlists_idx), True)
